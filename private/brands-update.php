...
$expected = array('brand_name', 'brand_id', 'brand_file');
...
if (array_key_exists('update', $_POST)) {
  ...
  $sql = "UPDATE sb_brands SET brand_name = '$brand_name', brand_file = '$brand_file'
...
$fileFieldName = "brand_file";
$fileUploadDir = 'brands';
...
<form id="theform" name="theform" enctype="multipart/form-data" method="post" action="" >
	...
	echo '<p>Image:<br />';
	echo '<img class="brand-image-thumb" src="'.$filepath.'" /><br />'; 
	// hidden field with data
	echo '<input id="brand_file" name="brand_file" type="hidden" size="25" value="'.$row[$fileFieldName].'" />'; // TODO: change value to new file
	// show upload form
	...
	echo "</p>";
	?>
    <p><label for="brand_name">Brand Name:</label><br /><input id="brand_name" name="brand_name" type="text" size="25" value="<?php echo htmlentities($row['brand_name']); ?>" /></p>
    <p><input type="submit" name="update" value="Update Entry" /></p>
	<p><input name="brand_id" type="hidden" value="<?php echo $row['brand_id']; ?>" /></p>
</form>
<?php } ?>
<!-- END UPDATE FORM -->
