...
if (array_key_exists('insert', $_POST)) {
  ...
  $expected = array('link_name','link_file');
  ...
  $sql = "INSERT INTO sb_links (link_name, link_file)
          VALUES('$link_name','$link_file')";
  // process the query
...
<form id="theform" name="theform" method="post" enctype="multipart/form-data" action="" >
    ...
    <p><small>NOTE: Images size is 600px X 600px</small></p>
        <p><label for="link_file">Link Image:</label><br /><input type="hidden" name="MAX_FILE_SIZE" value="<?php echo MAX_FILE_SIZE; ?>" /><input type="file" name="image" id="image" /></p>
    <?php
	// if the "upload image" button has been submitted, display result
	if (isset($result)) {
	  echo "<p><strong>$result</strong></p>";
	  echo "<p><input type=\"hidden\" name=\"link_file\" value=\"$poo\" /></p>";
	  echo "<p><img src=\"../links/$poo\" /></p>";
	  }
	?>
    <p><input type="submit" name="upload" id="upload" value="Upload" /></p>
    <p><b>STEP 02) Insert Data</b></p>
    <p><label for="link_name">link Name:</label><br /><input id="link_name" name="link_name" type="text" size="25" /></p>
